function create_tri_vertices(mesh, dim)
  ...
  vertices = Matrix{Float64}(undef, 3, length(elements) * 3)

  for i in eachindex(elements)
    vertex = nodes[:, elements[i]]
    vertices[1:3, 3*(i-1)+1:3*i] = vertex
  end

  centerX = sum(vertices[1, :]) / size(vertices, 2)
  centerY = sum(vertices[2, :]) / size(vertices, 2)
  centerZ = sum(vertices[3, :]) / size(vertices, 2)
...
function create_line_vertices(mesh, dim)
  nodes = mesh.nodes
  elements = mesh.connectivity[(dim, 0)]
  vertices = Matrix{Float64}(undef, 3, length(elements) * 2)

  for i in eachindex(elements)
    vertex = nodes[:, elements[i]]
    vertices[1:3, 2*(i-1)+1:2*i] = vertex
  end
